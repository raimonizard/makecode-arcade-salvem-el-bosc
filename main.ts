...
function chooseLevel () {
    ...
    if (settingLevel == 1 && difficulty == 0) {
        ...
        bforestA = sprites.create(assets.image`forestA`, SpriteKind.button)
        bforestB = sprites.create(assets.image`forestB`, SpriteKind.button)
        bforestA.setPosition(30, 75)
        bforestB.setPosition(130, 70)
        fire_plane = sprites.create(assets.image`Fire Plane 2 Left`, SpriteKind.Player)
...
sprites.onOverlap(SpriteKind.monkey, SpriteKind.button, function (sprite3, otherSprite2) {
    otherSprite2.startEffect(effects.halo, 1000)
    monkey2.sayText("A per confirmar", 1000, false)
    if (otherSprite2 == red_plane && controller.A.isPressed()) {
        fire_plane.setImage(assets.image`Fire Plane 2 Right`)
        effects.clearParticles(otherSprite2)
        choosePlane()
    } else if (otherSprite2 == ryanair_plane && controller.A.isPressed()) {
        fire_plane.setImage(assets.image`ryanair`)
        effects.clearParticles(otherSprite2)
        choosePlane()
    } else if (otherSprite2 == vueling_plane && controller.A.isPressed()) {
        fire_plane.setImage(assets.image`vuelingair`)
        effects.clearParticles(otherSprite2)
        choosePlane()
    } else if (otherSprite2 == aalines_plane && controller.A.isPressed()) {
        fire_plane.setImage(assets.image`aalines`)
        effects.clearParticles(otherSprite2)
        choosePlane()
    }
})
...
function choosePlane () {
    ...
    if (set_plane == 1) {
        ...
        monkey = sprites.create(assets.image`monkey-player`, SpriteKind.monkey)
