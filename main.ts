...
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if (setting_level != 1) {
        sprites.spray(fire_plane, assets.image`water`)
...
controller.left.onEvent(ControllerButtonEvent.Pressed, function () {
    if (setting_plane == 1) {
        ...
        animation.runImageAnimation(
        fire_plane,
        assets.animation`red_plane_left`,
        300,
        true
        )
...
function choose_plane () {
    ...
    if (setting_plane == 1) {
        ...
        music.play(music.stringPlayable("E E F G G F E D ", 110), music.PlaybackMode.LoopingInBackground)
        game.splash("Escull un avió")
        red_plane = sprites.create(assets.image`Fire Plane 2 Right`, SpriteKind.button)
        red_plane.setPosition(50, 20)
        ryanair_plane = sprites.create(assets.image`ryanair`, SpriteKind.button)
        ryanair_plane.setPosition(75, 52)
        vueling_plane = sprites.create(assets.image`vuelingair`, SpriteKind.button)
        vueling_plane.setPosition(120, 85)
        aalines_plane = sprites.create(assets.image`aalines`, SpriteKind.button)
        aalines_plane.setPosition(85, 112)
        monkey = sprites.create(assets.image`monkey-player`, SpriteKind.monkey_type)
...
controller.right.onEvent(ControllerButtonEvent.Pressed, function () {
    if (setting_plane == 1) {
        ...
        animation.runImageAnimation(
        fire_plane,
        assets.animation`red_plane_right`,
        300,
        true
        )
    } else if (chosen_plane == 2) {
        animation.runImageAnimation(
        fire_plane,
        assets.animation`ryanair_right`,
        300,
        true
        )
    } else if (chosen_plane == 3) {
        animation.runImageAnimation(
        fire_plane,
        assets.animation`vueling_right`,
        300,
        true
        )
    } else if (chosen_plane == 4) {
        animation.runImageAnimation(
        fire_plane,
        assets.animation`aalines_right`,
        300,
        true
        )
    }
...
sprites.onOverlap(SpriteKind.monkey_type, SpriteKind.button, function (sprite3, otherSprite2) {
...
function choose_level () {
    ...
    if (setting_level == 1 && chosen_level == 0) {
        ...
        forest_a = sprites.create(assets.image`forestA`, SpriteKind.button)
        forest_b = sprites.create(assets.image`forestB`, SpriteKind.button)
