...
def on_left_pressed():
    if setting_plane == 1:
        animation.run_image_animation(monkey, assets.animation("""
            monkey_left
        """), 300, True)
    elif chosen_plane == 1:
...
def choose_plane():
    ...
    if setting_plane == 1:
        ...
        music.play(music.string_playable("E E F G G F E D ", 110),
...
def on_on_overlap(sprite3, otherSprite2):
    global fire_plane, chosen_plane, setting_plane
...
def choose_level():
    global forest_a, forest_b, setting_level
    tiles.set_tilemap(tilemap("""
        level1
    """))
